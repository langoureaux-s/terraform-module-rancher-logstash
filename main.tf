...
data "template_file" "docker_compose_logstash" {
  ...
  vars {
    ...
    filebeat_crt              = "${var.filebeat_certificate != "" ? "LS_CERT_FILEBEATCRT_NAME: filebeat.crt\n${indent(6, "LS_CERT_FILEBEATCRT_CONTEND")}: |\n${indent(8, var.filebeat_certificate)}" : "" }"
    filebeat_key              = "${var.filebeat_key != "" ? "LS_CERT_FILEBEATKEY_NAME: filebeat.key\n${indent(6, "LS_CERT_FILEBEATKEY_CONTEND")}: |\n${indent(8, var.filebeat_key)}" : "" }"
    label_scheduling          = "${var.label_scheduling}"
    global_scheduling         = "${var.global_scheduling}"
    ports                     = "${length(var.ports) > 0 ? "ports:\n ${indent(6, join("\n", formatlist("- %s", var.ports)))}" : ""}"
